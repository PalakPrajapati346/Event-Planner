...
st.set_page_config(page_title="Event Planner - CrewAI", page_icon="🧠", layout="wide")
...
def init_db():
    ...
    # Create History/Plans Table LINKED to Users
    # Notice the 'username' column? That is how we separate data.
...
def get_user_history(username):
    ...
    # This Query filters data so users ONLY see their own history
    c.execute('''SELECT event_name, location, budget, timestamp, crew_output 
                 FROM history 
                 WHERE username = ? 
...
def save_plan_to_db(username, event, location, budget, content):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    ...
    c.execute('''INSERT INTO plans 
                 (username, event_name, location, budget, plan_content, timestamp) 
                 VALUES (?, ?, ?, ?, ?, ?)''', 
              (username, event, location, budget, str(content), timestamp))
    conn.commit()
    conn.close()

def get_user_history(username):
    """Fetch all plans generated by the logged-in user."""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT event_name, location, budget, timestamp, plan_content FROM plans WHERE username = ? ORDER BY id DESC', (username,))
    data = c.fetchall()
    conn.close()
    return data

# --- CREWAI SETUP (Your Original Logic) ---
def load_env():
...
def get_serper_api_key():
    load_env()
    return os.getenv("SERPER_API_KEY", "YOUR_SERPER_KEY_HERE")
...
def append_crewai_output_to_history(
    ...
    # Check if file exists to decide if we need to initialize with an empty list
...
# Read your API key from the environment variable
    ...
    with open(".env", "w") as f:
        f.write("GEMINI_API_KEY=AIzaSyAXaqnbSoIUrUnnpBnEJsXAdSeBNZ1gcI8\n")
    with open(".env", "w") as f:
        f.write("SERPER_API_KEY=6bc3c782b34632817f5b997bd97c9badcabb7d07\n")
    import os
    os.environ["GEMINI_API_KEY"] = "AIzaSyAXaqnbSoIUrUnnpBnEJsXAdSeBNZ1gcI8"
    ...
    # ✅ Use the model from litellm.yaml
    ...
    st.sidebar.markdown("## 📜 Past Events")
...
# Fetch only THIS user's history
    ...
    if user_history:
        ...
        # row structure: 0=event, 1=loc, 2=budget, 3=time, 4=output
            event_title = f"{row[0]} ({row[3].split(' ')[0]})"
        
            with st.sidebar.expander(event_title):
                st.markdown(f"**📍 Location:** {row[1]}")
                st.markdown(f"**💰 Budget:** ${row[2]}")
            
            # Button to download/view the full AI plan again
                st.download_button(
                    label="📥 Download Plan",
                    data=row[4],
                    file_name=f"{row[0]}_Plan.txt",
                    mime="text/plain",
                    key=f"hist_{row[3]}" # Unique key using timestamp
                )
    else:
        st.sidebar.info("No history found. Generate your first plan!")
...
if not st.session_state["logged_in"]:
    ...
    if choice == "Login":
        st.header("Login to Event Planner")
        ...
        if st.button("Signup"):
            if add_user(new_user, new_password):
                ...
                st.info("Navigate to Login from the sidebar.")
            else:
